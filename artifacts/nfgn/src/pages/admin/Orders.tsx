...
export function AdminOrdersPage() {
  ...
  const { data, isLoading, refetch } = useListOrders({ page, limit: 20, status: status !== "all" ? status : undefined });
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          ...
                          <Badge variant={order.paymentStatus === "demo_paid" ? "default" : "secondary"}>{order.paymentStatus?.replace("_", " ")}</Badge>
                        </div>
                        <p className="text-sm text-muted-foreground">{order.userName} • {order.paymentMethod}</p>
                        <p className="text-xs text-muted-foreground">{new Date(order.createdAt).toLocaleString()}</p>
                        {order.shippingAddress && <p className="text-xs text-muted-foreground mt-1">📍 {order.shippingAddress}</p>}
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="font-bold text-lg">${order.total.toFixed(2)}</p>
                        <p className="text-xs text-muted-foreground">Subtotal ${order.subtotal.toFixed(2)}</p>
                        <Select value={order.status} onValueChange={s => handleStatusChange(order.id, s)}>
                          <SelectTrigger className="w-32 h-7 text-xs mt-2">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
